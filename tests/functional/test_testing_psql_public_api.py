...
def test_testpsql_without_db_config() -> None:
    ...
    assert testdb.postgres.master_db == DBConfig(
        host="localhost", name="postgres", user="postgres", password="password", port=5433
    )
    assert testdb.options.namespace is None
    ...
    assert testdb.postgres.testdb == DBConfig(
        host="localhost", name="tmp_testdb", user="postgres", password="password", port=5433
    )
    testdb.stop()


def test_testpsql_with_db_config() -> None:
    # Let's run container to simulate Development DB
    devpostgres_container = PostgresDockerContainer(
        port=5444, options=ContainerOptions(name="dev-db", should_stop=True, remove_on_stop=True)
    )
